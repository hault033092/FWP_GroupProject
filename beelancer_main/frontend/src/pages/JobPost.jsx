...
const JobPost = () => {
  ...
  return <div style={{ backgroundColor: '#d6d6d6' }}>
    ...
    <PostForm>
      ...
      <p className='introQuote'>Our dilligent and hardworking freelancers will get your work done. Sweet as honey!!</p>
      <form action="" onSubmit={handleSubmit}>
        <TextBox>
          <h2>Name Of Project</h2>
          <input type="text" id="name" name="name" placeholder='Game of throne audio book, etc..' />
        </TextBox>
        <TextBox>
          <h2>Description</h2>
          <textarea name="description" id="description" placeholder='We need emotional voices...' onChange={(e) => { setCount(400 - e.target.value.length) }}></textarea>
        ...
        <TextBox>
          ...
          <div className='imageBox'>
              ...
              selectedImages.map((image, index) => {
                return (
                  <div key={image} className="previewHolder">
                    <button onClick={() => setSelectedImages(selectedImages.filter((e) => e != image))} className='quitButton'>✖</button>
                    <img src={image} />
                  </div>
                );
              })
            ...
            <label>
              <input className='imageInput' type="file" name="images" onChange={onSelectedFile} multiple accept='image/png,image/jpeg,image/webp'></input>
          ...
          <div className='skillBox'>
            {
              skills.map((s) => {
                return (
                  <SkillStyle>
                    <p>{s}</p>
                    <button onClick={() => { setSkills(skills.filter((e) => e != s)) }}>✖</button>
                  </SkillStyle>);
              })
            }
          </div>
          <div className='skillHolder'>
            <input className='skillInput' type="text" id="skill" name="skill" placeholder='C# programming, etc...' onChange={(e) => { setSkillInput(e.target.value) }} />

            <button className='skillEnter' onClick={addASkill}>Add A Skill</button>
          </div>
        </TextBox>
        <PaymentBox>
          <h2>Payment</h2>
          <PaymentSection>
            <Link className='paymentLink' to="/byHour" style={{ textDecoration: 'none' }}>
            ...
            <Link className='paymentLink' to="/fixedPrice" style={{ textDecoration: 'none' }}>
